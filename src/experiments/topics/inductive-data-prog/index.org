...
4. Zip all the files required for the experiment and upload them. The name of the zip file should be inductive-data-prog.zip.
#+END_EXAMPLE
...
+ Step 3 - Specify the if-else condition using the keyword /cond/.
  - If /n/ is 0, return 1.
  - Else multiply /n/ with the result of the recursive function /!/
...
+ Step 3 - Specify an if-else condition using the keyword /cond/. 
  ...
  - Else
    ...
    + Call the function /list-sum/ recursively by passing the rest of
      the list /ls/ as argument using the keyword /rest/. 
